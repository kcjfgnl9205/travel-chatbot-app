...
| 폴백 블록 [콜백 사용] | ⚠️ v1.1 배포로 **ON** 이어야 한다. 2026-09-12 테스트에서는 꺼진 문구가 나왔다 (2-③) |
| 대표 명령어 | `호텔`·`항공권`·`여행지` 있음 — 서버가 `fromCommand` 로 모델 없이 처리 |
| 검색 소요 시간 | ⚠️ 27~75초. 콜백 수명(5분) 안이지만 **사용자가 그만큼 기다린다** |
...
카카오 규칙: **5초 안에 대기 응답**, 그 뒤 콜백 URL 은 **최대 5분·1회** 유효
(오픈빌더 콜백 설정 화면 명시).

~~1분 안에 못 보내면 버려진다~~ → **5분이므로 지금 소요 시간도 예산 안에는 들어간다.**
그래도 줄여야 하는 이유는 **사용자가 그 시간을 그대로 기다리기** 때문이다. 75초는 단톡방
대화 흐름에서 너무 길다.
...
> ⚠️ 우선순위가 한 번 바뀌었다. 콜백 수명이 1분인 줄 알고 A-2 를 최우선으로 뒀는데,
> 실제로는 **5분**이라 "카드가 버려진다" 는 진단은 틀렸다. 지금 A-2 의 목적은
> 장애 복구가 아니라 **체감 대기 시간 단축**이다.
...
⚠️ 콜백 URL 은 **1회만** 유효하다. 결과든 실패든 한 번만 푸시해야 한다 (지금 코드도
