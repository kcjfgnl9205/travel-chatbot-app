...
고지 말풍선은 **할 말이 있을 때만** 붙는다 — 무시한 조건·출발지 추정 둘뿐이다.
둘 다 없으면 카드 하나만 나간다. (예전에는 "AI가 정리한 참고 정보예요" 를 항상 달았는데
매 카드마다 같은 문장이 반복돼 뺐다)
...
**사용자에게는 알리지 않는다** — 할 수 있는 일이 없는 사정이고, 다음 질문에는 새
결과가 나간다. 아무것도 못 주는 것보다 낫다.
...
고지는 카드 **뒤에** 오는 별도 말풍선이고, **할 말이 있을 때만** 붙는다.
header 40자에는 안 들어가고, 앞에 세우면 결과를 가린다.
