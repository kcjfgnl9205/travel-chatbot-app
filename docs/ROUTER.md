...
"베트남 여행지 추천해줘" 는 `kind: 'country'` 로 해석되고, **검색 대신 그 나라의 도시를
되묻는다.**

```
베트남 어디로 가세요?
도시를 고르면 관광지를 찾아드릴게요.
[다낭 관광지] [하노이 관광지] [호치민 관광지] [나트랑 관광지]
```

나라 단위로 검색하면 다낭·하노이·호치민이 섞인 목록이 나와 아무에게도 쓸모가 없다.
⚠️ 되묻되 **그 나라의 도시**를 보여준다 — 예전에는 베트남을 물어도 오사카·도쿄를
권했는데, 그건 되묻기가 아니라 딴소리다.

도시 목록은 모델에게 한 번 받아 **각각 place 로 등록하고 나라에 매단다**(`parent_id`).
그래서 같은 나라를 다시 물으면 DB 조회 한 번으로 끝난다 — 나라의 대표 도시는 시간이
지나도 변하지 않으므로 TTL 을 두지 않는다.

```
베트남(country) ─┬─ 다낭(city) ─── 미케비치(area)
                 ├─ 하노이(city)
                 └─ 호치민(city)
```
