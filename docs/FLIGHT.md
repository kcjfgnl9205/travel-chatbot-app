...
## 1.5 ⚠️ 카드 모양 — 그룹챗봇은 itemCard 를 못 그린다

**팀톡방에서 항공권만 아무 메시지도 안 떴다.** 호텔·관광지는 같은 방에서 멀쩡했다. 차이는 하나뿐이었다 — 항공권만 `carousel` + `itemCard` 를 썼다.

오픈빌더 블록 편집기의 봇 응답 선택지에도 itemCard 가 없다(텍스트·이미지·카드·커머스·리스트뿐). 관리자센터 봇 테스트 창에서는 렌더링되지만 실제 팀톡방에서는 말풍선이 통째로 사라진다.

그래서 **기본 카드 모양이 `listCard` 다** (`FLIGHT_CARD_STYLE=list`). 호텔·관광지와 같은 모양이 된다.

### 무엇을 잃었나

| | itemCard 캐러셀 | listCard (지금 기본) |
|---|---|---|
| 카드 수 | 10장 | **5줄** |
| 한 편에 쓸 수 있는 자리 | 5줄 (key 6자 + value 20자) + 강조 가격 | 제목 40자 + 설명 40자 |
| 왕복 가는편/오는편 | 각각 한 줄 | 한 줄에 **출발 시각만** 압축 |
| 소요 시간 | 항상 | 편도만 |

줄은 이렇게 접었다.

```
제목  대한항공 KE723 · 325,000원      ← 가격을 제목에. 편을 고르는 첫 번째 축이다
설명  10/3(토) 09:20 ↔ 10/7(수) 12:30 · 직항
```

왕복은 도착 시각과 소요 시간을 버린다. 대신 **직항 여부는 남긴다** — 경유가 몇 번인지가 시각 다음으로 중요하고, 예약 페이지를 열기 전에 알아야 거를 수 있기 때문이다. 나머지는 줄 링크(예약 페이지)로 넘긴다.

### 안내 말풍선은 그대로 둔다

`listCard` header 는 40자라 노선·조건·경고가 안 들어간다. 그래서 캐러셀 때와 똑같이 앞에 `simpleText` 를 세운다(outputs 2개). 그 말풍선이 들고 가는 게 둘 있고, 둘 다 없애면 안 된다.

- **출발지를 추측했다는 사실** — 부산에서 가려던 사람이 고쳐 말할 유일한 단서
- **가격이 확정 운임이 아니라는 말** — 없으면 카드 가격을 믿고 눌렀다 배신당한다

⚠️ 팀톡방에서 출력이 **2개**인 게 문제인지는 확인하지 못했다. simpleText 도(대기 메시지) listCard 도(호텔) 그 방에서 각각 잘 나오는 건 확인됐지만 둘을 같이 보낸 적은 없다. 카드가 여전히 안 보이면 여기를 먼저 의심하고, 안내 문구를 header 로 접어야 한다.

### 되돌리는 법

`itemCard` 코드는 지우지 않았다. 일반 채널 챗봇에서는 정상 동작하므로, 채널을 따로 운영하게 되면 `FLIGHT_CARD_STYLE=carousel` 로 되돌린다. [flight-card-style.spec.ts](../test/flight-card-style.spec.ts) 가 그 탈출구가 살아 있는지 지킨다 — 되돌릴 수 없는 탈출구는 없는 것과 같다.
...
엔티티가 오면 모델에 같은 걸 다시 묻지 않는다 (더 빠르고 공짜). 보는 이름 — **한국어가 먼저다.** 오픈빌더 커스텀 엔티티는 한국어 이름을 그대로 파라미터 키로 쓴다.

| 필드 | 파라미터 이름 |
|---|---|
| 출발지 | `출발지` · `출발도시` · `여행도시1` · `origin` · `departure` · `from_city` |
| 도착지 | **`여행도시`** · `도착지` · `도착도시` · `목적지` · `여행지` · `도시` · `destination` · `arrival` · `to_city` · `city` · `sys_location` |
| 가는 날 | `출발일` · `가는날` · `날짜` · `depart_date` · `date` · `sys_date` |
| 오는 날 | `귀국일` · `오는날` · `return_date` |

지금 블록에 실제로 붙어 있는 건 **`여행도시` 하나**다. 출발지는 태깅돼 있지 않으니 도시가 두 개 넘어오길 기대하면 안 된다.

엔티티가 안 왔을 때는 모델을 부르기 전에 [도시 사전](../src/modules/nlu/city-table.ts)을 본다 (0ms·0원). 단, **출발지를 말한 흔적("부산에서", "김포 출발")이 있으면 사전을 건너뛴다** — "오사카에서 서울 가는 비행기" 를 사전에 맡기면 더 긴 별칭인 오사카가 도착지로 잡혀 노선이 뒤집힌다. 어느 쪽이 출발지인지 가리는 건 모델의 일이다.
