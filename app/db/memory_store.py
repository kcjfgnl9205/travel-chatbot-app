...
def put(click_id: str, payload: dict[str, Any]) -> None:
    _clicks[click_id] = {**payload, "click_count": 0}
    _clicks.move_to_end(click_id)
    while len(_clicks) > _MAX_ENTRIES:
        _clicks.popitem(last=False)


def register_click(click_id: str) -> dict[str, Any] | None:
    """DB 의 register_click() 과 같은 계약: 카운터를 올리고 행을 돌려준다."""
    entry = _clicks.get(click_id)
    if entry is None:
        return None
