 ...
 * 도시 사전 (237개).
 *
 * ⚠️ **이 표는 지명의 정본이 아니다.** 지역의 신원은 DB `places` 행(`place_id`)이고,
 *    캐시 키도 그걸로 갈린다. 표에서 찾은 도시도 곧바로 upsert 돼 그 행을 얻는다
 *    ([places.service.ts](./places.service.ts) register). 표는 **씨앗이자 지름길**이다.
 *
 * 그래서 성격이 데이터라기보다 **모델 호출을 줄이는 캐시**에 가깝고, DB 가 아니라
 * 코드에 있다 — 요청 경로(카카오 5초 예산)에서 0ms 로 읽혀야 하고, DB 가 흔들려도
 * 지명 인식은 살아 있어야 하며, 아래 예외 목록들은 데이터가 아니라 판단이라
 * 코드 리뷰와 git blame 이 붙어야 한다.
 *
 * 읽는 곳이 둘이다. 둘 다 요청 경로이고, 둘 다 표에 걸리면 **모델을 안 부른다.**
 *
 *   1. **문장 파서** — [intent.service](../intent/intent.service.ts) `fromKeywords` 가
 *      `findCityInText` 로 발화에서 도시를 훑는다. 걸리면 의도 파싱이 0ms·0원으로 끝난다.
 *   2. **지명 정규화** — [places.service](./places.service.ts) `resolve` 4단계 중 3단계.
 *      `lookupCity` 로 전체 일치를 본다. **공항 코드(IATA)가 여기서 딸려 온다** —
 *      항공권 프롬프트의 `인천(ICN) → 오사카(KIX)` 가 이 값이고, 모델 경로로 등록된
 *      도시에는 없을 수 있다.
 *
 * 표기를 슬러그로 모아두는 것도 중요하다. "동경"·"Tokyo"·"도쿄" 가 한 행으로 안 모이면
 * 같은 도시를 물어도 캐시가 갈려 AI 검색이 새로 돈다.
 *
 ...
 *   "사파리 투어 있어?"    → 베트남 사파
 *
 * 막는 건 문장 경로뿐이다 — lookupCity 로 "빈" 이 통째로 들어오면 그건 도시가 맞다.
...
/**
 * 문자열 전체가 도시인가. **이미 뽑힌 지명 하나**를 표준화할 때 쓴다
 * ([places.service.ts](./places.service.ts) `resolve` · `resolveParent`).
 *
 * 부분 일치가 아니다 — "오사카 호텔" 은 여기서 null 이다. 여기 들어오는 값은 모델이나
 * 문장 파서가 이미 지명으로 골라낸 것이라 전체 일치로 보는 게 맞고, 발화에서 훑어내는
 * 일은 findCityInText 가 맡는다.
 */
