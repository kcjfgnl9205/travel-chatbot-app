...
import { KakaoSkillPayload, utteranceOf } from '../kakao/skill-payload';
import { CITIES, hasCity, parse } from '../nlu/nlu';
import { SkillTokenGuard } from './skill-token.guard';

@Controller('api/v1/kakao')
@UseGuards(SkillTokenGuard)
export class KakaoHotelController {
  private readonly logger = new Logger(KakaoHotelController.name);
